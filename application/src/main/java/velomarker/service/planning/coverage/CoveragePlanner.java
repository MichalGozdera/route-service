...
public class CoveragePlanner {
    ...
    /** Porządkowanie obszarów krzywą Hilberta (bbox liczony per plan()). */
    private final HilbertOrdering ordering = new HilbertOrdering();
    ...
    /** STRZAŁY/plan per faza (debug-only, gated debugGeoJson) — akumulator per plan(). */
    private Map<String, Long> shots;
    /** Instrumentacja wall-time seeda (log-only) — akumulowana w INIT-GROW, raportowana na końcu. */
    ...
    public CoverageResult plan(UUID taskId,
        ...
        Map<String, Double> rewardPerCategory = RewardModel.rewardPerCategory(candidatePool);
    ...
    private void logHoleDiagnostics(RouteCalculation bestCalc, List<UnvisitedArea> candidatePool,
                                    GminaIndex gminaIndex, Set<Integer> bestVisited) {
     ...
     */
    private List<SeedSel> seedScoreAndOrder(List<UnvisitedArea> pool, Map<String, Double> rewards,
                                            List<double[]> baseline, GminaIndex gminaIndex,
     ...
     */
    private double seedCompactLoop(List<double[]> route, List<SeedSel> selected, List<double[]> anchorOnly,
            List<double[]> anchors, List<double[]> baseline, double[] baseCum, EdgeCache cache,
            BiFunction<List<Waypoint>, String, RouteCalculation> brouter, String profile, double alphaKmPerMeter,
            GminaIndex gminaIndex, List<UnvisitedArea> pool, Map<String, Double> rewards, double hiBand,
            double targetEffort, java.util.function.Function<double[], UnvisitedArea> findGminaCached,
            double realEffort, int[] growNearAcc) {
     ...
     * i liczbę dopiętych.
     */
    private HoleFillResult seedHoleFill(List<double[]> route, List<SeedSel> selected, List<double[]> anchorOnly,
     ...
     */
    private double seedTrim(List<double[]> route, List<SeedSel> selected, List<double[]> anchorOnly,
            List<double[]> anchors, List<double[]> baseline, double[] baseCum, EdgeCache cache,
            BiFunction<List<Waypoint>, String, RouteCalculation> brouter, String profile, double alphaKmPerMeter,
            GminaIndex gminaIndex, List<UnvisitedArea> pool, Map<String, Double> rewards, double hiBand,
            double targetEffort, java.util.function.Function<double[], UnvisitedArea> findGminaCached,
            double realEffort, int[] trimmedAcc, int[] growNearAcc) {
    ...
    void greedySeedRoute(List<double[]> route, List<double[]> anchors, GminaIndex gminaIndex,
        ...
        List<SeedSel> scored = seedScoreAndOrder(pool, rewards, baseline, gminaIndex, EFFORT_MULTIPLIER);
        ...
        final int round = 0;
        {
            ...
            for (int islPass = 0; islPass < 3; islPass++) {
                ...
                for (int i = 1; i < route.size() - 1; i++) {
                    ...
                    boolean island = failedEdges.contains(GeometryUtil.edgeKey(route.get(i - 1), cur))
                            || failedEdges.contains(GeometryUtil.edgeKey(cur, route.get(i + 1)));
        ...
        realEffort = seedCompactLoop(route, selected, anchorOnly, anchors, baseline, baseCum, cache,
                brouter, profile, alphaKmPerMeter, gminaIndex, pool, rewards, hiBand, targetEffort,
                findGminaCached, realEffort, growNearAcc);
        ...
        realEffort = seedTrim(route, selected, anchorOnly, anchors, baseline, baseCum, cache,
                brouter, profile, alphaKmPerMeter, gminaIndex, pool, rewards, hiBand, targetEffort,
                findGminaCached, realEffort, trimmedAcc, trimGrowAcc);
        ...
        HoleFillResult hf = seedHoleFill(route, selected, anchorOnly, anchors, baseline, baseCum, cache,
                brouter, profile, alphaKmPerMeter, gminaIndex, pool, targetEffort, findGminaCached, realEffort);
        ...
        do {
            ...
            while (changed && pass < maxPasses + 6) {
                ...
                for (Cand c : cands) {
                    ...
                    RelocResult relocResult = relocateShallowDeferred(route, selected, baseline, baseCum, cache, brouter, profile,
                            alphaKmPerMeter, gminaIndex, Set.of(spurArea.areaId()), prev, cur, next, idx, spurArea, eIn, eOut,
                ...
                if (!toDelete.isEmpty()) {
                    ...
                    cache.setReason("pomiar");
                    List<double[]> realTrack = concatRealGeometry(route, cache, brouter, profile, alphaKmPerMeter);
                    ...
                    for (int vid : delGids) {
                        ...
                        EdgeCache.EdgeInfo bestLegEdge = getEdge(route.get(bestLeg), route.get(bestLeg + 1), cache, brouter, profile, alphaKmPerMeter);
                        double[] heartPoint = heart.clone();
                        seedSlicedEdgesAtPoint(cache, bestLegEdge, route.get(bestLeg), route.get(bestLeg + 1), bestSeg, heartPoint, alphaKmPerMeter);
            ...
            reroutedLegs = rerouteApproximateLegs(route, cache, brouter, profile, alphaKmPerMeter); // realny re-route sliced legów → ujawnia wtórniaki
            if (debugGeoJson) {
    ...
    private GrowNearResult growNear(List<double[]> route, List<SeedSel> selected, List<double[]> anchorOnly,
        ...
        for (SeedSel s : allInserts) {
            ...
            if (pos < 0) continue;
            boolean fail = failedEdges.contains(GeometryUtil.edgeKey(route.get(pos - 1), s.point()))
                    || failedEdges.contains(GeometryUtil.edgeKey(s.point(), route.get(pos + 1)));
            if (fail) {
                selected.remove(s);
    ...
    private void anchorResetTouched(List<double[]> route, List<SeedSel> selected, List<double[]> anchorOnly,
        ...
        while (foundUncoveredDeep && iteration < 5) {
            ...
            realTrack = concatRealGeometry(route, cache, brouter, profile, alphaKmPerMeter);
